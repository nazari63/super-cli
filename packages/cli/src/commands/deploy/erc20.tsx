...
import {
	getChainByID,
	getChainByNetworkIdentifier,
	initChainConfig,
	SupportedNetwork,
} from '@/utils/superchainRegistry';
...
function getDeploymentPublicClients(
	chains: string[],
	l1Id: SupportedNetwork,
): PublicClient[] {
...
function getDeploymentSignerClient(
...
const DeployErc20Command = (props: Props) => {
	...
	const [deterministicAddresses, setDeterministicAddresses] = useState<
		Address | '0x'
	>('0x');
	const {
		addDeployment,
	...
	const deployment = useDeploymentsStore(
		state => state.deployments[deterministicAddresses],
	);
	...
	useEffect(() => {
		(async () => {
			...
			const artifact = await readForgeArtifact(forgeArtifactPath);
			...
			const superchainClients = getDeploymentPublicClients(
				chains.split(','),
			...
			for (let superchainClient of superchainClients) {
				const isErc20AlreadyDeployed = await superchainClient.getCode({
				...
				updateDeploymentStepStatus({
					...
					message: isErc20AlreadyDeployed
			...
			for (let superchainClient of superchainClients) {
				...
				const hash = await deployCreate2Contract({
					client: getDeploymentSignerClient(
						account,
						superchainClient.chain!.id,
					),
					salt: creationSalt32Bytes,
					initCode: initCode,
				});

				const receipt = await superchainClient.waitForTransactionReceipt({
					hash,
				});
